...
class JoinRelationParser extends Parser
{
     ...
     * @throws \Exception
     */
    public function makeQuery(Builder $builder, $rule, $queryCondition = 'AND')
    {
        $value = $this->getValueForQueryFromRule($rule);
        $condition = strtolower($queryCondition);

        if (is_array($this->joinFields) && array_key_exists($rule->attribute, $this->joinFields)) {
            return $this->buildSubclauseQuery($builder, $rule, $value, $condition);
        }

        return $this->convertQBToQuery($builder, $rule, $value, $condition);
    }
    ...
    private function buildSubclauseQuery($builder, $rule, $value, $condition)
    {
        ...
        $subclause = $this->joinFields[$rule->attribute];
        $subclause['operator'] = $operator;
        $subclause['value'] = $value;
        $subclause['require_array'] = $this->operatorRequiresArray($operator);
        $not = array_key_exists('not_exists', $subclause) && $subclause['not_exists'];

        /**
         *  "from_table" => "documents"
         *  "from_col" => "assignee_id"
         *  "to_table" => "users"
         *  "to_col" => "id"
         *  "to_value_column" => "name"
         *  "operator" => "="
         *  "value" => "Lia Dibbert"
         *  "require_array" => false
         *  ?"not_exists" => false
        ...
        $builder = $builder->whereHas($subclause['relation'], function (Builder $query) use ($subclause) {

            // "select 1 from `users` where users.id = documents.assignee_id and `documents`.`deleted_at` is null"
            $q = $query->selectRaw(1)
            ...
            //"select 1 from `users` where users.id = documents.assignee_id and `name` = ? and `documents`.`deleted_at` is null"
